...
pixmap::trim()
{
	...
	size_t new_width = width_ - (left_margin + right_margin);
	size_t new_height = height_ - (top_margin + bottom_margin);

	size_t pixel_size = get_pixel_size();
	uint8_t *new_bits = new uint8_t[new_width*new_height*pixel_size];

	const uint8_t *p = &bits_[(top_margin*width_ + left_margin)*pixel_size];
	uint8_t *q = new_bits;

	for (size_t i = 0; i < new_height; i++) {
		::memcpy(q, p, new_width*pixel_size);

		p += width_*pixel_size;
		q += new_width*pixel_size;
	}
