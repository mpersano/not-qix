...
{
	const int TICS_PER_SEGMENT = 2;

	if (state_tics_%TICS_PER_SEGMENT == 0) {
		int index = state_tics_/TICS_PER_SEGMENT - 1;
...
player::draw() const
{
	if (state_ == state::IDLE || state_ == state::SLIDING || state_ == state::EXTENDING_IDLE || state_ == state::EXTENDING) {
		// trail
